...
# Ported from slime (SGLang backend) to vime (vLLM backend).
import re
...
def _vllm_finish_reason_type(output: dict, choice: dict) -> str:
    """Normalise vLLM finish_reason to a string type ('stop', 'length', 'abort')."""
    fr = choice.get("finish_reason") or "stop"
    if isinstance(fr, dict):
        return fr.get("type", "stop")
    if fr in ("length", "abort", "stop", "cancelled"):
        return "stop" if fr == "stop" else ("abort" if fr in ("abort", "cancelled") else "length")
    return "stop"


def _parse_vllm_response(output: dict, state: GenerateState, sampling_params: dict):
    """Parse token ids, logprobs, and decoded text from a vLLM /inference/v1/generate response."""
    ...
    text = state.tokenizer.decode(token_ids, skip_special_tokens=skip_decode) if token_ids else ""
    finish_type = _vllm_finish_reason_type(output, choice)
    return token_ids, log_probs, text, finish_type, choice


def _build_meta_info(finish_type: str, output: dict) -> dict:
    """Build a meta_info dict compatible with Sample.update_from_meta_info()."""
    meta: dict[str, Any] = {"finish_reason": {"type": finish_type}}
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    """Custom generation function supporting tool calls (vLLM backend)."""
    ...
    loss_masks: list[int] = sample.loss_mask
    ...
    # Build base sampling params once (max_new_tokens will be clamped per-turn).
    ...
    for turn in range(TOOL_CONFIGS["max_turns"]):
        ...
        per_turn_params = dict(inference_sampling_params)
        per_turn_params["max_tokens"] = min(
            inference_sampling_params.get("max_tokens", remaining_budget),
            remaining_budget,
        )

        # Use token IDs instead of text (vLLM token_ids workflow)
        current_token_ids = prompt_tokens_ids + response_token_ids
        payload = {
            "model": args.hf_checkpoint,
            "token_ids": current_token_ids,
            "sampling_params": per_turn_params,
        }
        ...
        # Parse vLLM response
        cur_response_token_ids, cur_log_probs, cur_response, finish_type, choice = _parse_vllm_response(
            output, state, sampling_params
        )
        ...
        # vLLM should always return token_ids; if we got none, abort for safety.
        ...
        # Inline what was previously sample.append_response_tokens(..., trainable=True)
        sample.tokens += cur_response_token_ids
        sample.response_length += len(cur_response_token_ids)
        if sample.loss_mask is None:
            sample.loss_mask = [1] * (sample.response_length - len(cur_response_token_ids))
        sample.loss_mask += [1] * len(cur_response_token_ids)
        if sample.rollout_log_probs is None:
            sample.rollout_log_probs = []
        sample.rollout_log_probs += cur_log_probs

        # Build meta_info and update sample status
        meta = _build_meta_info(finish_type, output)
        ...
        # Add dummy log probs for observation tokens (loss_mask=0 so they won't be used)
        response_token_ids += obs_tokens_ids

        # Inline what was previously sample.append_response_tokens(..., trainable=False)
        sample.tokens += obs_tokens_ids
        sample.response_length += len(obs_tokens_ids)
        sample.loss_mask += [0] * len(obs_tokens_ids)
    ...
    # Final status update (only if still PENDING after the loop)
