...
- Integration with language model generation via vLLM
- Reward calculation for tool usage

## Files

- `generate_with_retool.py`: Main generation function with tool support (vLLM backend)
- `tool_sandbox.py`: Tool execution and safety management
- `rl_data_preprocess.py`: Process RL dataset
...
## Differences from the slime version

- Rollout backend: **vLLM** (`/inference/v1/generate`) instead of SGLang (`/generate`)
- `GenerateState` imported from `vime.rollout.vllm_rollout` instead of `slime.rollout.sglang_rollout`
- CLI args: `--vllm-mem-fraction-static` instead of `--sglang-mem-fraction-static`
- Shell: `source /root/vime/scripts/models/` instead of `source /root/slime/scripts/models/`
- PYTHONPATH includes `/root/vime` instead of `/root/slime`
- The `append_response_tokens()` method (slime-specific) has been inlined for vime compat
